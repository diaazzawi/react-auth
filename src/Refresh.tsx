...
function Refresh<T>({
  ...
  refresh,
  store,
}: PropsWithChildren<RefreshProps<T>>): ReactNode {
  ...
  const refreshApiCall = () => {
    refresh
      ...
      .catch(() => {
        ...
        store.set(doSignOut());
        throw new AuthError(
          "Some error occured on the Refresh API. Unable to refresh for now",
        );
      });
  };
  ...
  useEffect(() => {
    ...
    // Else If refresh condition met
      ...
      value.isUsingRefreshToken &&
      value.refresh &&
      value.refresh.expiresAt > new Date()
    ) {
  ...
  // useInterval(refreshApiCall, store.value.isSignIn ? refresh.interval : null);
  useInterval(refreshApiCall, refresh.interval);
